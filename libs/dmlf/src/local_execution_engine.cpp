...
LocalExecutionEngine::PromiseOfResult LocalExecutionEngine::CreateExecutable(
    Target const &host, Name const &execName, SourceFiles const &sources)
{
  if (!HasTarget(host))
  {
  ...
  {
    return MakeErrorResult(ErrorCode::BAD_EXECUTABLE,
                           std::string{"Executable name" + execName + " already exists"});
  }
...
LocalExecutionEngine::PromiseOfResult LocalExecutionEngine::DeleteExecutable(
    Target const & /*host*/, Name const &execName)
{
  auto exec = executables_.find(execName);
  if (exec == executables_.end())
  {
    return MakeErrorResult(ErrorCode::BAD_EXECUTABLE,
                           std::string{"Executable " + execName + " doesn't existed"});
  }
...
LocalExecutionEngine::PromiseOfResult LocalExecutionEngine::CreateState(Target const &host,
                                                                        Name const &  stateName)
{
  if (HasState(host, stateName))
  {
    return MakeErrorResult(ErrorCode::BAD_STATE,
                           std::string{"State name" + stateName + " already exists"});
  }
...
LocalExecutionEngine::PromiseOfResult LocalExecutionEngine::CopyState(Target const &host,
                                                                      Name const &  srcName,
                                                                      Name const &  newName)
{
  ...
  {
    return MakeErrorResult(ErrorCode::BAD_STATE,
                           std::string{"State name" + srcName + " doesn't exist"});
  }
  if (HasState(host, newName))
  {
    return MakeErrorResult(ErrorCode::BAD_STATE,
                           std::string{"State name" + newName + " already exists"});
  }
...
LocalExecutionEngine::PromiseOfResult LocalExecutionEngine::DeleteState(Target const & /*host*/,
                                                                        Name const &stateName)
{
  ...
  {
    return MakeErrorResult(ErrorCode::BAD_STATE,
                           std::string{"State " + stateName + " doesn't exist"});
  }
  states_.erase(state);
...
LocalExecutionEngine::PromiseOfResult LocalExecutionEngine::Run(Target const &     host,
                                                                Name const &       execName,
                                                                Name const &       stateName,
                                                                std::string const &entrypoint)
{
  ...
  {
    std::string err_msg =
        std::accumulate(runtime_error.begin(), runtime_error.end(), std::string{""});
  ...
  FulfillPromise(
      promise, ExecutionResult{output, Error{ErrorStage::ENGINE, ErrorCode::SUCCESS, std::string{}},
                               std::string{}});
...
LocalExecutionEngine::PromiseOfResult LocalExecutionEngine::MakeErrorResult(ErrorCode   err_code,
                                                                            std::string err_msg)
{
...
LocalExecutionEngine::PromiseOfResult LocalExecutionEngine::FulfillPromise(PromiseFulfiller promise,
                                                                           ExecutionResult  res)
{
  serializers::MsgPackSerializer serializer;
  serializer << res;
